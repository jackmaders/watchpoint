...
marker/sync seam, although the marker scheduler would still be application code.
...
controls, but it is not a YouTube player or a marker scheduler. Its YouTube story
...
but it is old, promise-based, and still leaves React lifecycle and marker timing
...
marker windows. The package removes most iframe/API lifecycle code but does not
...
small TanStack Start integration and leaves no usable player seam for marker
...
- Add **Media Chrome** if controls/theme work is the main cost; do not expect it
  to remove the marker scheduler or the YouTube adapter.

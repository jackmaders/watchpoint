...
- Framework runtime entrypoints (`src/app/client.tsx`, `src/app/server.ts`,
  and `src/app/start.ts`), route files, and browser tests are explicit entry
  boundaries. The Vite, Vitest, Playwright, Drizzle, Biome, Wrangler, and
  Lefthook plugins discover their configured files under `.config`.

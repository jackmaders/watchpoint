...
- **Specialized Slice Entrypoints (`index.*.ts`):** In addition to canonical `index.ts`, specialized barrels (such as `index.server.ts` for server-only code and `index.async.ts` for deferred client code) are first-class slice entrypoints. A specialized barrel is completely valid on its own even if a generic `index.ts` is omitted (e.g., when a slice is exclusively server-oriented or exclusively loaded asynchronously).
...
- **Specialized Entrypoint Architecture (`index.*.ts`):**
  - Paralleling `index.server.ts` for server-only code, slices that support deferred/code-split client loading expose a dedicated public entrypoint: `index.async.ts`.
  - An `index.*.ts` entrypoint is an architecturally valid barrel on its own; a generic `index.ts` is not strictly required if a slice only provides specialized (e.g. async-only or server-only) exports.
  - When both eager and lazy variants are supported, `index.ts` exports the synchronous/eager component, while `index.async.ts` exports the self-suspending, dynamic-import component under the same canonical name (`export { SessionPanel } from "./ui/session-panel.async"`).

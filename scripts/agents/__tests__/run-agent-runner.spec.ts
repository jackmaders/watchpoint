...
import { z } from "zod";
import { RunAgentError, runAgent, type SpawnedProcess } from "../run-agent";

// The pure functions (parseStreamLine, substitutePromptArgs, extractTagged,
// parseAndValidate, classifyFailure) are tested directly, with no doubles,
// in run-agent.spec.ts. This file is the `spawn` seam and the real
// OUTPUT_DIR files — the two lower-level seams from the spec's testing
// decisions (§6).
...
describe("runAgent", () => {
	...
	describe("prompt templating", () => {
		it("substitutes promptArgs and injects {{OUTPUT_SCHEMA}} from the Zod schema", async () => {
			...
			await runAgent({
				cli: "gemini",
				model: "flash",
				output: { kind: "object", schema: ResultSchema, tag: "result" },
	...
	describe("usage and failure-reason reporting", () => {
		...
		it("writes the failure class and message to failure_reason.txt on failure", async () => {
			// Arrange
			const { proc } = createFakeProcess({ exitCode: 53 });
			const spawn = vi.fn().mockReturnValue(proc);
		...
		it("still appends usage, but writes no failure_reason.txt, for the unclassified exit-1 case", async () => {
			// Arrange
			// The 7th, outside-the-table exit path (§5.1 exit 1, no quota text)
			// still logs usage on every run, but failure_reason.txt is
			// deliberately skipped — there is no FailureClass to report, and
			// the thrown Error's own message carries the diagnostic instead.
			const { proc } = createFakeProcess({
				exitCode: 1,
				stderrChunks: ["network timeout"],
			});
